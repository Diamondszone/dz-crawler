...
# OUTPUT_GH_TOKEN: untuk push hasil ke crawler-8k/8k
if [ -n "${GH_TOKEN:-}" ]; then
  echo "https://${GH_TOKEN}:x-oauth-basic@github.com" > ~/.git-credentials
  chmod 600 ~/.git-credentials || true
fi

# ===== Lokasi kerja kode (Nixpacks → /app, BUKAN repo git) =====
export REPO_DIR="${REPO_DIR:-/app}"
mkdir -p "$REPO_DIR/results" "$REPO_DIR/state" "$REPO_DIR/warc_paths_cache"

# ===== Repo OUTPUT 8k (hanya folder WARC + WP/WIX .txt di ROOT) =====
...
while true; do
  echo "[run] batch…"
  # ganti nama file jika bukan 8000.py
  ...
  echo "[stage] siapkan output bersih ke repo 8k"
  # bersihkan root _out/ (kecuali .git)
  find "$OUT_DIR" -mindepth 1 -maxdepth 1 ! -name ".git" -exec rm -rf {} +
  # salin hanya WP-site*.txt & WIX-site*.txt ke ROOT (dengan struktur folder WARC)
